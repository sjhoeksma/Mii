 ...
 *
 * It has been modified to work with either one
 ...
 *******************************************************************************
    ...
    Copyright (C) 2010  Chris Parish, S.J.Hoeksma (minimized)
...
ShiftLCD::ShiftLCD(uint8_t dp, uint8_t cp, uint8_t lp)
{
  init (dp, cp, lp);
}


void ShiftLCD::init(uint8_t dp, uint8_t cp, uint8_t lp)
{
  _data_pin = dp;
  _clock_pin = cp;
  _latch_pin = lp;

  _backlight &= ~LCD_BL_PIN; //Turn blacklight on by default
  ...
  //begin(16, 2); //Set default display, placed here to reduce size
...
void ShiftLCD::createChar(uint8_t location, const uint8_t charmap[]) {
  location &= 0x7; // we only have 8 locations 0-7
  command(LCD_SETCGRAMADDR | (location << 3));
  if (inSRAM(&charmap[0])){
    for (byte i=0; i<8; i++) { write(charmap[i]); }
  } else {
    for (byte i=0; i<8; i++) { write(pgm_read_byte(&charmap[i])); }
  }
}
...
void ShiftLCD::write4bits(uint8_t value, uint8_t mode) {
  uint8_t EN_SWITCH = B00000010;
  uint8_t RS_SWITCH = B00000001;
  uint8_t cmd = 0;
  uint8_t data = 0;
  if (!mode) {
    cmd = 0 | _backlight;
