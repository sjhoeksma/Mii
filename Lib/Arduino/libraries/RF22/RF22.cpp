...
//	Serial.println(_bufLen);
	...
	_rxHeaderTo = spiRead(RF22_REG_47_RECEIVED_HEADER3);
	_rxHeaderFrom = spiRead(RF22_REG_48_RECEIVED_HEADER2);
	_rxHeaderId = spiRead(RF22_REG_49_RECEIVED_HEADER1);
	_rxHeaderFlags = spiRead(RF22_REG_4A_RECEIVED_HEADER0);
	#if RF22_STATS
	_rxGood++;
	#endif
	_bufLen = len;
...
//	Serial.println("IPREAVAL");
	_lastRssi = (int8_t)(-120 + ((spiRead(RF22_REG_26_RSSI) / 2)));
	resetRxFifo();
	clearRxBuf();
    }
}
...
void RF22::setTxPower(uint8_t power)
{
   ...
   // spiWrite(RF22_REG_6D_TX_POWER, power | RF22_TXPOW_LNA_SW);
}
...
boolean RF22::setModemConfig(ModemConfigChoice index)
{
  ...
  #ifdef RF22_FIXED_CONFIG
    const  RF22::ModemConfig cfg = RF22_FIXED_CONFIG;
...
{
    if (!_rxBufValid) {
      if (_mode==RF22_MODE_TX) return false;
...
{
    while (_mode == RF22_MODE_TX){idle();}; // Wait for any previous transmit to finish
    //if (!waitPacketSent(250)) setModeIdle();
}
