...
#define RF_MIN_MESSAGE_INTERVAL 65

//Setting modem config to fixed will save 600 bytes
//#define RF22_FIXED_CONFIG { 0x2b, 0x03, 0xf4, 0x20, 0x41, 0x89, 0x00, 0x36, 0x40, 0x0a, 0x1d, 0x80, 0x60, 0x10, 0x62, 0x2c, 0x22, 0x08 } //FSK 2, 5

//Should we created statistic, 1 if yes
#define RF22_STATS 1

//Set to one if we should do a SPI isolation
#define RF22_SPI_ISOLATE 1

//Should we swap RX/TX spins
#define RF22_PIN_SWAP 0
...
#define RF22_MAX_MESSAGE_LEN 55
#endif
...
public:
    ...
    boolean        recv(uint8_t* buf=0, uint8_t* len=0);
    ...
    virtual boolean        send(const uint8_t* data, uint8_t len);
    ...
    virtual void idle(){}
    virtual void wait(uint32_t time=RF_MIN_MESSAGE_INTERVAL){uint32_t t=time+millis();while (millis()<t){idle();}}
