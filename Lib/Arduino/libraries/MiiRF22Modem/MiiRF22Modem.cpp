...
// 2 RF22s per Arduino
MiiRF22Modem* MiiRF22Modem::_deviceForInterrupt = 0;
...
void MiiRF22Modem::handleInterrupt(){
    ...
    if (_lastInterruptFlags[0] & MII_RF_IPKVALID) {
	    ...
	    if (   len >  MII_RF_MAX_MESSAGE_LEN || len < _bufLen){
	       _mode = MII_RF_MODE_IDLE;
	       clearRxBuf();
...
void MiiRF22Modem::setTxPower(uint8_t power){
...
bool MiiRF22Modem::internalRecv(uint8_t* buf, uint8_t* len){
...
bool MiiRF22Modem::appendTxBuf(const uint8_t* data, uint8_t len){
    if (((uint16_t)_bufLen + len) > MII_RF_MAX_MESSAGE_LEN)
	return false;
    ATOMIC_BLOCK_START;
...
void MiiRF22Modem::readNextFragment(){
    if (((uint16_t)_bufLen + MII_RF_RXFFAFULL_THRESHOLD) > MII_RF_MAX_MESSAGE_LEN)
