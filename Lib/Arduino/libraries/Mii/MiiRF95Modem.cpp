...
PROGMEM static const MiiRF95Modem::ModemConfig MODEM_CONFIG_TABLE[] =
{
    ...
    { 0x64,   0x84,    0x00}, // Bw62_5Cr45Sf256
...
void MiiRF95Modem::powerOn(bool state){
  if (_sdnPin!=MII_NO_PIN) {
    ...
    delay(25);
    //Now Enable it again
    if (state) digitalWrite(_sdnPin, HIGH);
  }
  //This modem needs a long wait before powering up
  delay(500);
}
...
bool MiiRF95Modem::init(uint8_t address,uint16_t syncWords,uint8_t channel,uint8_t power,bool isMaster,ModemConfigChoice index){
    ...
    attachInterrupt(interruptNumber, isr, RISING);
...
void MiiRF95Modem::validateRxBuf() {
    ...
    if (_promiscuous || (_rxHeaderTo == _thisAddress && _syncWords== _rxHeaderSyncWords) || _rxHeaderTo == MII_BROADCAST_ADDRESS) {
   	  _rxTime = millis();
    	_rxBufValid = true;
    }
}
...
bool MiiRF95Modem::internalRecv(uint8_t* buf, uint8_t* len,bool peek){
  ...
  if (!peek) clearRxBuf(); // This message accepted and cleared
  return true;
}
...
void MiiRF95Modem::setTxPower(int8_t power){
  power=(uint8_t)map(power,MII_TXPOW_1DBM,MII_TXPOW_23DBM,5,23);
